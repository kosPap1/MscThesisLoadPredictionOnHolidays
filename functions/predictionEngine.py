...
def predictionEngine1(loadTrainDayTarget, loadTrainDayMinus1, loadTrainDayMinus7, loadTrainDayMinus365, loadTestDayTarget,
    ...
    MLPreg = MLPRegressor(max_iter=2000).fit(trainSetX, trainSetY.ravel())
    ...
    modelSVM = svm.SVR()
    modelSVM.fit(trainSetX, trainSetY.ravel())
    y_predSVM = modelSVM.predict(testSetX)
    ...
    y_predSVM = scaler.inverse_transform(y_predSVM / scaler_coeff)
    ...
    mape3 = (mean_absolute_percentage_error(testSetY, y_predSVM)) * 100
    print('MAPE with SVM is ' + str(mape3) + ' for ' + str(filename))
    print('\n')
