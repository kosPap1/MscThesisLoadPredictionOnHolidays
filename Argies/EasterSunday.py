...
from functions.dataInput import load2016, load2015, load2018, load2019, temp2016, temp2019
...
# Model training
model = xgb.XGBRegressor(n_estimators=50, learning_rate=0.09, max_depth=3, base_score=4, booster='dart',
                         subsample=0.75, reg_lambda=0.6)
...
MLPreg = MLPRegressor(hidden_layer_sizes=(213), random_state=0, activation='tanh', solver='sgd',
...
modelSVM = svm.SVR(kernel ='linear',C= 6.7595, degree=2)
...
print('MAPE for Easter Sunday with XGBoost is ' + str(mean_absolute_percentage_error(y_pred, testY) * 100))
print('MAPE for Easter Sunday with MLP is ' + str(mean_absolute_percentage_error(y_pred_MLP, testY) * 100))
print('MAPE for Easter Sunday with SVM  is ' + str(mean_absolute_percentage_error(y_predSVM, testY) * 100))
